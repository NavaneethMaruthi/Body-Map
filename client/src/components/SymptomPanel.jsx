...
export default function SymptomPanel({ region, onSaved, onCancel }) {
  ...
  const fetchInsight = async (sev, cat, nte) => {
    setAiLoading(true);
    try {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSaved(true);
      onSaved();
      await fetchInsight(severity, category, notes);
    } catch (err) {
    ...
    <div className="p-6 flex flex-col gap-5 overflow-y-auto">
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-white font-semibold text-lg">
          {saved ? 'Symptom Saved ✓' : 'Log Symptom'}
        </h3>
        <button
          onClick={onCancel}
      ...
      {/* AI Insights — shown after saving */}
      {(aiLoading || insight) && (
        <div className="bg-gray-800 border border-teal-500/20 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-3">
            <span className="text-lg">🤖</span>
            <h4 className="text-teal-400 text-sm font-semibold">AI Health Insights</h4>
          </div>
          {aiLoading ? (
      ...
      {/* Done button — shown after insight loads */}
      {insight && (
        <button
          onClick={onCancel}
          className="w-full py-2.5 rounded-lg bg-teal-500 hover:bg-teal-400 text-white text-sm font-semibold transition-colors"
        >
          ✓ Done
        </button>
      )}
